...
def draw_3d_box_on_image(image, label_2_file, P2, c=(0, 255, 0)):
...
def draw_box_on_bev_image_v2(bev_image, points_filter, label_2_file, pred_path, calib_file, overall_new_boxes=None, gt_boxes=None, c = (0, 0, 255)):
    ...
    with open(pred_path) as f:
        for line in f.readlines():
            ...
            bev_image = cv2.putText(bev_image, str(round(location[2], 2)), (x_img[3]+50, y_img[3]), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
...
class Projector():
    def __init__(self, camera_view, camera_matrix, dist_coeffs):
    ...
    def project(self, points_cloud, image, rmat, tvec):
        ...
        image_points = lidar_to_image(points_cloud, rmat, tvec, self.camera_matrix).squeeze()
