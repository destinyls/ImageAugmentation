...
def create_kitti_info_file(kitti_root,
    ...
    total_test_img_ids = _read_imageset_file("datasets/kitti/ImageSets/test.txt")
    print("Generate info. this may take several minutes.")

    test_img_ids = []
    for image_id in total_test_img_ids:
        image_name = "{:06d}".format(image_id) + ".png"
        img_name = os.path.join("datasets/kitti/testing", "image_2", image_name)
    ...
    kitti_infos_test = kitti.get_kitti_image_info(
        ...
        relative_path=relative_path)  
    filename = info_path / 'kitti_infos_test_monoflex_baseine_trainval.pkl'
    print(f"Kitti info val file is saved to {filename}")
    with open(filename, 'wb') as f:
...
def create_groundtruth_database(kitti_root,
    ...
    if info_path is None:
        info_save_path = root_path / 'kitti_infos_test_monoflex_baseine_trainval.pkl'
    else:
        path_info = pathlib.Path(info_path)
        path_info.mkdir(parents=True, exist_ok=True)
        info_save_path = path_info / 'kitti_infos_test_monoflex_baseine_trainval.pkl'
    if database_save_path is None:
        database_save_path = root_path / 'gt_database_monoflex_baseine_trainval'
    else:
        database_save_path = pathlib.Path(database_save_path)
    database_save_path.mkdir(parents=True, exist_ok=True)

    if info_path is None:
        db_info_save_path = root_path / "kitti_dbinfos_monoflex_baseine_trainval.pkl"
    else:
        db_info_save_path = path_info / "kitti_dbinfos_monoflex_baseine_trainval.pkl"
...
if __name__ == "__main__":
    ...
    create_groundtruth_database(kitti_root, info_path="kitti_infos", database_save_path="gt_database_monoflex_baseine_trainval")
